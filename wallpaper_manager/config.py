...
class Config:
    ...
    fps: int = 30
    """Max FPS for the wallpaper to keep performance lightweight."""
    ...
    """Wallpaper-specific properties (e.g. colors, alignment).
    Key is wallpaper ID, value is a dict of property-value pairs."""

    disable_gl_threaded_optimizations: bool = False
    """Set __GL_THREADED_OPTIMIZATIONS=0 env var to work around OpenGL glitches."""
...
def save_config(config: Config, path: Path = CONFIG_FILE) -> None:
    """Save the configuration back to TOML."""
    lines = [
        '# Auto-generated by Wallpaper Manager Configuration GUI',
        f'binary = "{config.binary}"',
        f'assets_dir = "{config.assets_dir}"',
        f'workshop_dir = "{config.workshop_dir}"',
        f'wallpaper = "{config.wallpaper}"',
        f'scaling = "{config.scaling}"',
        f'screen = "{config.screen}"',
        f'fps = {config.fps}',
        ...
        '',
        '[resilience]',
        f'max_retries = {config.resilience.max_retries}',
        f'retry_delay_seconds = {config.resilience.retry_delay_seconds}',
        '',
    ]
    
    if config.properties:
        for wp_id, props in config.properties.items():
            lines.append(f'[properties."{wp_id}"]')
